...
```
覆盖范围：
├── specs/          4 份核心规范（架构/核心库/模块/质量）
├── extensions/     12 个扩展库文档（SqlSugar/Redis/JWT/EventBus…）
└── problems/       168 个已验证的编译/运行时问题解决方案
```
...
```
覆盖范围：
├── work-order.md          工单创建/派发/报工/关闭
├── equipment.md           设备台帐/维修/保养操作
├── quality-inspection.md  来料/过程/出货检验流程
├── material.md            物料收发/库存管理
├── production-reporting.md 生产报工/进度跟踪
├── traceability.md        产品追溯操作
└── reports.md             统计报表查询
```
...
```
覆盖范围：
├── patterns/        4 种页面模式（搜索表格/树形表格/抽屉编辑/详情Tab）
├── components/      7 个组件用法（ApiSelect/DatePicker/Upload…）
├── checklist.md     代码审查清单
└── known-issues.md  已知问题解决方案
```
...
# 3. 还原依赖（NuGet + Lzq.Extensions 私有源）
dotnet restore

# 4. 配置 appsettings.json（数据库连接、Redis、AI API Key 等）
