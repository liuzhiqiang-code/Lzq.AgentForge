...
import {
  ...
  getRepairPage,
  startRepair,
} from '#/modules/equipment/api/repair';
...
function onStart(row: RepairApi.RepairItem) {
  AntModal.confirm({
    title: $t('ui.actionMessage.confirmTitle', [$t('equipment.repair.start'), row.code || '']),
    content: '',
...
function onAccept(row: RepairApi.RepairItem) {
  AntModal.confirm({
    title: $t('ui.actionMessage.confirmTitle', [$t('equipment.repair.accept'), row.code || '']),
    content: '',
    okText: $t('common.confirm'),
    cancelText: $t('common.cancel'),
    onOk: () => {
...
function onCancel(row: RepairApi.RepairItem) {
  AntModal.confirm({
    title: $t('ui.actionMessage.confirmTitle', [$t('equipment.repair.cancel'), row.code || '']),
    content: '',
    okText: $t('common.confirm'),
    cancelText: $t('common.cancel'),
    okType: 'danger',
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('equipment.repair.list')">
      <template #toolbar-tools>
        <Button type="primary" @click="onCreate">
          <Plus class="size-5" />
          {{ $t('ui.actionTitle.create', [$t('equipment.repair.name')]) }}
        </Button>
      </template>
    </Grid>
  </Page>
