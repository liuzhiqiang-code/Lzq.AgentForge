...
import { getEquipmentStatusOverview } from '#/modules/mes/api';
  ...
  <Card class="shadow-sm" :loading="loading">
    <template #title>
      <div class="flex items-center">
        ...
        {{ $t('mes.dashboard.equipmentStatus') }}
      </div>
    </template>
    <div class="grid grid-cols-2 gap-2">
      <div class="text-center p-2 bg-green-50 rounded">
        <div class="text-xl font-bold text-green-600">{{ running }}</div>
        <div class="text-sm text-gray-600">{{ $t('mes.dashboard.running') }}</div>
      </div>
      <div class="text-center p-2 bg-red-50 rounded">
        <div class="text-xl font-bold text-red-600">{{ stopped }}</div>
        <div class="text-sm text-gray-600">{{ $t('mes.dashboard.stopped') }}</div>
      </div>
      <div class="text-center p-2 bg-yellow-50 rounded">
        <div class="text-xl font-bold text-yellow-600">{{ maintenance }}</div>
        <div class="text-sm text-gray-600">{{ $t('mes.dashboard.maintenance') }}</div>
      </div>
      <div class="text-center p-2 bg-gray-50 rounded">
        <div class="text-xl font-bold text-gray-600">{{ scrapped }}</div>
        <div class="text-sm text-gray-600">{{ $t('mes.dashboard.scrapped') }}</div>
      </div>
    </div>
  </Card>
</template>
