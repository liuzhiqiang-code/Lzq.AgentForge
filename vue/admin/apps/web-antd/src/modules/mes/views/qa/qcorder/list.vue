...
function getStatusActionLabel(code: string): string {
  const labels: Record<string, string> = {
    submitInspect: $t('mes.qa.qcorder.submitInspect'),
    judgePass: $t('mes.qa.qcorder.judgePass'),
    judgeFail: $t('mes.qa.qcorder.judgeFail'),
    cancel: $t('mes.qa.qcorder.cancel'),
  };
...
function getStatusActions(row: QCOrderApi.QCOrderItem): Array<{ code: string; label: string }> {
  ...
  if (s === 0) actions.push(
    { code: 'submitInspect', label: $t('mes.qa.qcorder.submitInspect') },
  );
  if (s === 1) actions.push(
    { code: 'judgePass', label: $t('mes.qa.qcorder.judgePass') },
    { code: 'judgeFail', label: $t('mes.qa.qcorder.judgeFail') },
  );
  if (s === 0) actions.push({ code: 'cancel', label: $t('mes.qa.qcorder.cancel') });
  return actions;
}
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('mes.qa.qcorder.list')">
      <template #toolbar-tools>
        <Button type="primary" @click="onCreate">
          <Plus class="size-5" />
          {{ $t('ui.actionTitle.create', [$t('mes.qa.qcorder.name')]) }}
        </Button>
      </template>
