...
import {
  ...
  pauseWorkOrder,
  startWorkOrder,
} from '#/modules/workorder/api/workorder';
...
function getStatusActionLabel(code: string): string {
  const labels: Record<string, string> = {
    dispatch: $t('workorder.workorder.dispatch'),
    start: $t('workorder.workorder.start'),
    complete: $t('workorder.workorder.complete'),
    close: $t('workorder.workorder.close'),
    cancel: $t('workorder.workorder.cancel'),
    pause: $t('workorder.workorder.pause'),
  };
...
function getStatusActions(row: WorkOrderApi.WorkOrderItem): Array<{ code: string; label: string }> {
  ...
  if (s === 0) actions.push({ code: 'dispatch', label: $t('workorder.workorder.dispatch') });
  if (s === 1) actions.push({ code: 'start', label: $t('workorder.workorder.start') }, { code: 'cancel', label: $t('workorder.workorder.cancel') });
  if (s === 2) actions.push({ code: 'complete', label: $t('workorder.workorder.complete') }, { code: 'pause', label: $t('workorder.workorder.pause') });
  if (s === 6) actions.push({ code: 'start', label: $t('workorder.workorder.resume') });
  if (s === 3) actions.push({ code: 'close', label: $t('workorder.workorder.close') });
  return actions;
}
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('workorder.workorder.list')">
      <template #toolbar-tools>
        <Button type="primary" @click="onCreate">
          <Plus class="size-5" />
          {{ $t('ui.actionTitle.create', [$t('workorder.workorder.name')]) }}
        </Button>
      </template>
      <!-- 状态操作列 -->
