...
function getStatusActions(row: DefectApi.DefectItem): Array<{ code: string; label: string }> {
  ...
  if (s === 0) actions.push({ code: 'handle', label: $t('qa.defect.handle') });
  if (s === 1 && row.needReview) actions.push({ code: 'review', label: $t('qa.defect.review') });
  return actions;
}
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('qa.defect.list')">
