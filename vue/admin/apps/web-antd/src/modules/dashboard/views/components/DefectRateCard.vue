...
import { getDefectRateStatistics } from '#/modules/dashboard/api';
...
<template>
  <Card class="shadow-sm" :loading="loading">
    ...
    <Statistic
      ...
      :title="$t('dashboard.todayDefectRate')"
      suffix="%"
    />
    <div class="mt-2 flex items-center justify-between">
      <span class="text-sm text-gray-600">{{ $t('dashboard.weekTrend') }}</span>
      <Tag :color="getTrendColor(weekTrend)">
        {{ weekTrend === 'up' ? '↑' : weekTrend === 'down' ? '↓' : '→' }}
        {{ $t('dashboard.weekTrend') }}
      </Tag>
    </div>
  </Card>
</template>
