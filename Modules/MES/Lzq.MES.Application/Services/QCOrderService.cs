...
public class QCOrderService : ServiceBase, IQCOrderService
{
    ...
    public async Task<ApiResult<QCOrderViewDto>> GetAsync(long id)
    {
        ...
        var cacheKey = string.Format(RedisKeys.QCOrderGet, id);
    ...
    {
        var redis = GetRequiredService<ILzqRedisClient>();
        var cacheKey = string.Format(RedisKeys.QCOrderItems, qcOrderId);
    ...
    public async Task<ApiResult<bool>> UpdateAsync([FromBody] QCOrderUpdateCommand command)
    {
        ...
        // 清除缓存
        var redis = GetRequiredService<ILzqRedisClient>();
        await redis.RemoveAsync(string.Format(RedisKeys.QCOrderGet, command.Id));
        await redis.RemoveAsync(string.Format(RedisKeys.QCOrderItems, command.Id));
    ...
    public async Task<ApiResult<bool>> DeleteAsync(long id)
    {
        ...
        // 清除缓存
        var redis = GetRequiredService<ILzqRedisClient>();
        await redis.RemoveAsync(string.Format(RedisKeys.QCOrderGet, id));
        await redis.RemoveAsync(string.Format(RedisKeys.QCOrderItems, id));
